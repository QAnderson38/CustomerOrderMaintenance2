		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("BusinessPartnerID") === oObject.BusinessPartnerID;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].BusinessPartnerID;
									} else {
										sCurrentId = aItemsNotInTheList[0].BusinessPartnerID;
									}
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("BusinessPartnerID"),
						title: oBindingContext.getProperty("CompanyName")
					};
				},
				iPressTheAddButton: function() {
					return this.waitFor({
						...
						errorMessage: "Could not find the addButton button"
					});
				}
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function() {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "BusinessPartnerSet (0)"
						}),
						success: function() {
							Opa5.assert.ok(true, "The list header displays 'BusinessPartnerSet (0)'");
						},
				...
				theListShouldBeSortedAscendingOnName: function() {
					return this.theListShouldBeSortedAscendingOnField("CompanyName");
				},
				...
				theListShouldHaveAllEntries: function() {
					...
					// retrieve all BusinessPartnerSet to be able to check for the total amount
				...
				iShouldSeeTheNoDataTextForNoSearchResults: function() {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function(oList) {
							Opa5.assert.strictEqual(oList.getNoDataText(), oList.getModel("i18n").getProperty("masterListNoDataWithFilterOrSearchText"),
					...
					return this.waitFor({
						...
						success: function(oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "BusinessPartnerSet (" + iExpectedLength + ")"
								}),
								success: function() {
				...
				theAddButtonShouldBeEnabled: function() {
					return this.waitFor({
						viewName: sViewName,
						id: "addButton",
						matchers: new PropertyStrictEquals({
						...
						success: function() {
							Opa5.assert.ok(true, "The Add button is enabled");
						},
						errorMessage: "The Add button is disabled"
					});
				},
				theAddButtonShouldBeDisabled: function() {
					return this.waitFor({
						viewName: sViewName,
						id: "addButton",
						matchers: new PropertyStrictEquals({
						...
						success: function() {
							Opa5.assert.ok(true, "The Add button is disabled");
						},
